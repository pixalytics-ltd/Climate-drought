...
class SOILWATER:
    ...
    def __init__(self, args):
        ...
        # Setup logging
        self.logger = logging.getLogger("test_soil_water")
    ...
    def run_drought(self):
        ...
        self.logger.debug("Downloading soil water data for {sd} to {ed}.".format(sd=self.args.start_date, ed=self.args.end_date))

        exit_code = 0
        #try:

        # Need all times for accumulation
        self.args.accum = True # must always be true
        self.working_dir = self.args.outdir
        spi = era5_processing.Era5SoilWaterProcessing(self.args, self.working_dir) 
        ...
        #     self.logger.info("Downloading complete for '{}' completed.".format(output_file_path))
