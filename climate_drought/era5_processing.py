...
#         req_baseline = erq.ERA5Request(erq.SOILWATER_VARIABLES, True, 'soilwater', self.args, self.config, baseline=True)
#         dlf_baseline = exists_or_download(req_baseline)

#         # create era5 request object for short term period
#         self.args.accum = True
#         req_shorterm = erq.ERA5Request(erq.SOILWATER_VARIABLES, True, 'soilwater', self.args, self.config, baseline=True)
        

#         # submit the request to the era5 API via our ERA5 request module
#         spi = erq.ERA5Download(req_baseline,self.logger)
